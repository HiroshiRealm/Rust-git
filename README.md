...
*   `gc`: 执行仓库垃圾回收，将松散对象打包成 packfile 并应用增量压缩以节省空间。
...
*   `repository`：封装了所有对 Git 仓库的底层操作，是所有 Git 命令的基础。可以将其理解为一套操作 `.git` 目录的 API，它屏蔽了底层的复杂性，为上层命令提供了简洁的接口。其核心职责包括：
    *   **对象 (Objects) 数据库**: 负责所有 Git 对象（`commit`, `tree`, `blob`）的序列化、反序列化、zlib 压缩以及读写。
    *   **引用 (Refs) 管理**: 解析、更新和创建 `HEAD` 以及 `refs/` 目录下的所有分支和标签引用。
    *   **索引 (Index) / 暂存区**: 提供了对 `.git/index` 文件的完整读写能力，是 `add` 和 `status` 等命令的核心。
    *   **Packfile**: 实现了仓库垃圾回收（`gc`）时所需的对象打包、增量压缩和 `.idx` 索引文件的生成逻辑。
...
换言之，在我们的代码编写过程中就是，这个增量压缩，是直接嵌入在read_object/write_object的时候，还是只在gc (garbage collection)的时候实现.
...
答案在于这是一个精心设计的 **Tradeoff**。256这个数字并非随意选择，它直接对应了SHA-1哈希值第一个 **字节** 的256种可能性（0x00 到 0xff）。选择一个字节作为分区的依据，是在三个关键因素中取得的绝佳平衡：
...
本项目作为一个教学和演示目的的 Git 实现，仍有许多可以完善和扩展的地方。以下是一些已知的限制和未来可能的工作方向：

*   **有限的 `push` 和 `pull` 功能**:
    *   当前的 `push`、`pull` 和 `fetch` 命令只能对当前检出的分支进行操作。它们会将当前分支推送到远程仓库的同名分支，或从远程仓库的同名分支拉取更新。
    *   不支持 `git push origin local-branch:remote-branch` 这样的 refspec 语法，即无法指定本地分支和远程分支的对应关系。这是未来一个重要的扩展方向。

*   **缺失的 `clone` 命令**:
    *   项目目前没有实现 `git clone` 命令。用户需要手动 `init` 一个新仓库，然后通过 `remote add` 和 `pull` 来获取远程内容。实现一个完整的 `clone` 命令将极大提升可用性。

*   **尚未实现从 Packfile 读取**:
    *   虽然 `gc` 命令可以创建增量压缩的 `.pack` 和 `.idx` 文件，但我们的 Git 实现目前还**不能**从这些 packfile 中读取对象。这意味着一旦 `gc` 执行后，像 `cat-file` 或 `log` 这样的命令可能就无法找到被打包的对象了。
    *   实现从 packfile 中高效地查找和解压对象，是让 `gc` 功能真正变得实用的关键一步。
...
*   **简单的合并（Merge）策略**:
    *   目前的合并策略非常简单，当发生冲突时（例如，两个分支修改了同一个文件的同一部分），程序会直接报错退出。
    *   一个更完整的 Git应该能产生一个带有冲突标记（conflict markers）的工作目录，让用户可以手动解决冲突，然后完成合并。

*   **对标签（Tags）和 annotated tags 的支持**:
    *   项目目前只支持分支，没有实现对轻量标签（lightweight tags）和附注标签（annotated tags）的支持。

*   **更完善的命令行接口**:
    *   可以为各个子命令添加更多的选项和标志，使其行为更接近原版 Git。
