...
Git 的策略是"延迟"这种重度工作，并将其"批量处理"。它将昂贵的增量压缩操作推迟到以下场景：

- **`git gc` (Garbage Collection)**：当仓库对象积累到一定数量时，`gc` 命令会被触发。它会收集所有的松散对象，在它们之间智能地寻找最佳的增量压缩链，然后将结果打包到一个或几个大的 `.pack` 文件中。
- **`git push` / `git fetch`**：当向远程仓库推送或从远程仓库拉取时，Git 需要将传输的数据打包。在这个过程中，它也会执行增量压缩，以最小化网络传输的数据量。

**这种批量处理的优势是双重的：**
1.  **全局最优**：通过一次性分析大量的对象，Git 更有可能找到最优的压缩组合，从而获得比单个对象增量压缩更高的空间效率。
2.  **不影响日常操作**：将计算开销集中在少数不那么频繁的操作上，保证了开发者日常使用最频繁的命令的流畅性。

总而言之，Git 通过区分"松散对象"和"包文件（Packfile）"这两种存储方式，在 **操作速度** 和 **长期存储效率** 之间找到了一个绝佳的平衡点。它优先保证了开发者的即时体验，然后通过后台的、批量的垃圾回收和打包机制，来保证仓库长期的健康和高效。
