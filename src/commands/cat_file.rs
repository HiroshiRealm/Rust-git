...
pub fn execute(object_hash: &str) -> Result<()> {
    ...
    let git_object = crate::repository::objects::read_object(&repo, object_hash)?;

    match git_object.object_type.as_str() {
        "blob" => {
            ...
            print!("{}", String::from_utf8_lossy(&git_object.data));
        }
        "tree" => {
            let mut g_cursor = 0;
            while g_cursor < git_object.data.len() {
                // Find the space separating mode and name
                let g_space_idx = match git_object.data[g_cursor..].iter().position(|&b| b == b' ') {
                    Some(idx) => idx + g_cursor,
                    None => anyhow::bail!("Invalid tree object: missing space after mode"),
                };
                let g_mode_str = str::from_utf8(&git_object.data[g_cursor..g_space_idx])?;

                // Find the null byte terminating the name
                let g_nul_idx = match git_object.data[g_space_idx + 1..].iter().position(|&b| b == 0) {
                    Some(idx) => idx + g_space_idx + 1,
                    None => anyhow::bail!("Invalid tree object: missing null terminator after name"),
                };
                let g_name_str = str::from_utf8(&git_object.data[g_space_idx + 1..g_nul_idx])?;

                // The SHA-1 hash is the next 20 bytes
                let g_sha1_start = g_nul_idx + 1;
                let g_sha1_end = g_sha1_start + 20;
                if g_sha1_end > git_object.data.len() {
                    anyhow::bail!("Invalid tree object: insufficient data for SHA-1 hash");
                }
                let g_sha1_bytes = &git_object.data[g_sha1_start..g_sha1_end];
        ...
        _ => {
            anyhow::bail!("Unknown object type: {}", git_object.object_type);
        }
    }
